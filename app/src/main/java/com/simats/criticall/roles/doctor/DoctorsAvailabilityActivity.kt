...
class DoctorsAvailabilityActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnEnableWeekdays = findViewById(R.id.btnEnableWeekdays)
        ...
        // Load from API (keeps defaults if API fails)
        lifecycleScope.launch {
            val token = AppPrefs.getToken(this@DoctorsAvailabilityActivity).orEmpty()
    ...
    private fun renderDays() {
        ...
        days.forEach { d ->
            ...
            // Professional: pick start OR end individually (no double dialog glitch)
            ...
            tvEnd.setOnClickListener {
                ...
                pickTime(d.end) { picked ->
                    val newEnd = picked
                    if (!isEndAfterStart(d.start, newEnd)) {
            ...
            // Optional: clicking row opens Start picker only (still smooth)
    ...
    private fun saveAvailability() {
        ...
        // validate enabled days have valid ranges
        ...
        lifecycleScope.launch {
            ...
            if (ok) {
                ...
                // Stay on this screen after save to avoid “app closed” perception.
                // Doctor can go back manually.
            } else {
    ...
    private fun apiGetAvailability(token: String): JSONArray? {
        return try {
            ...
            json.optJSONArray("availability")
        } catch (_: Throwable) {
            null
        }
    }
